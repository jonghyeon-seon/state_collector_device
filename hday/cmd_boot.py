  ...
  def readVersion(self, timeout=500):
    ...
    if ret == True:
      ...
      if packet.err_code == 0:
        str_fmt = "<I4BHHHH2I32sI"
        fmt_size = calcsize(str_fmt)
        data = unpack(str_fmt, packet.data[:fmt_size])
        resp.boot.version_str = str(data[1]) + "." + str(data[2]) + "." + str(data[3])
        resp.boot.name_str = data[11].decode("utf-8")

        data = unpack(str_fmt, packet.data[fmt_size*1:fmt_size*2])
        resp.firm.version_str = str(data[1]) + "." + str(data[2]) + "." + str(data[3])
        resp.firm.name_str = data[11].decode("utf-8")

        data = unpack(str_fmt, packet.data[fmt_size*2:fmt_size*3])
        resp.update.version_str = str(data[1]) + "." + str(data[2]) + "." + str(data[3])
        resp.update.name_str = data[11].decode("utf-8")
